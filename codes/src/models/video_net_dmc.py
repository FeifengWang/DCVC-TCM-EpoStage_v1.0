...
class DMC(nn.Module):
    ...
    def get_y_bits_probs(y, sigma):
        ...
        # bits = (torch.log(probs+1e-5).sum() / (-math.log(2)))
        # (torch.clamp((torch.log(probs)/-math.log(2)), 0, 50)).sum()
    ...
    def TrainMotion(self):
        ...
        for p in self.optic_flow.parameters():
            p.requires_grad = True
        for p in self.mv_encoder.parameters():
            p.requires_grad = True
        for p in self.mv_prior_encoder.parameters():
            p.requires_grad = True
        for p in self.mv_prior_decoder.parameters():
            p.requires_grad = True
        for p in self.mv_decoder.parameters():
            p.requires_grad = True
        for p in self.bit_estimator_z_mv.parameters():
            p.requires_grad = True
    ...
    def SingleTrainstage(self, cur_epoch):
        if cur_epoch < 2:
            self.TrainMotion()
            motionmse = 1
            motionbpp = 0
            ...
            residualbpp = 1
            finalmse = 1
        return motionmse, motionbpp, residualbpp, finalmse

    def MultiTrainstage(self, cur_epoch):
        self.Trainall()
        motionmse = 0
        motionbpp = 1
        residualbpp = 1
        ...
        return motionmse, motionbpp, residualbpp, finalmse
    def forward_inter(self, x, ref_frame, ref_feature):
    ...
    def forward(self, frames):
        ...
        frames_likelihoods = []
        #
        # x_hat, likelihoods = self.forward_keyframe(frames[0]) #B,C,W,H
        ...
        for i in range(1, len(frames)):
            ...
            result_dict = self.forward_inter(x, x_ref, ref_feature)
            ref_feature = result_dict["feature"]
            x_ref = result_dict["recon_image"]
            mse_loss = result_dict["mse_loss"]
